...
// Generate one Effective M question
function generateEffectiveMQuestion() {
...
function EffectiveM() {
  ...
  const startNewQuestion = (diff) => {
    const data = generateEffectiveMQuestion();
  ...
  const handleSubmit = () => {
    ...
    if (guessInt === questionData.Effective_M) {
      setFeedback("Correct! Your guess is right.");
      setCorrect((c) => c + 1);
    } else {
      setFeedback(
        `Wrong! The correct Effective M-score is ${questionData.Effective_M}.`
      );
    }
  ...
  const handleTimeout = () => {
    ...
    finishQuestionTiming();
    setFeedback(
      `Time's up! The correct Effective M-score was ${questionData.Effective_M}.`
    );
    setTimeout(goNextOrEnd, 1500);
  };
    ...
    return (
      <div className="chips-trainer-page">
        ...
        <div className="chips-trainer-container">
          ...
          <p className="ct-note">
            Same setup as M-score, but scaled by number of players at the table
            (Effective M = M × players / 9).
          </p>
        </div>
      </div>
    );
        ...
        {questionData && !gameOver && (
          <>
            ...
            <div className="ct-input-row">
              <label htmlFor="em-guess">Your Effective M guess:</label>
              <input
                id="em-guess"
                type="number"
                value={userGuess}
